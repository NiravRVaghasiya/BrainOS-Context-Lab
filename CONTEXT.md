...
7. **`verify` and `without-extras` both run per change.** The suite is the
   ...
   guarantee about the development machine. Both jobs' commands were run by hand
   in this turn; the workflow itself needs Actions enabled in the repository
   settings (`gh api repos/…/actions/workflows` → `total_count: 0` today).
